...
def _nonnegative_float(value: object, default: float = 0.0) -> float:
    try:
        ...
        return round(max(0.0, parsed), 3) if math.isfinite(parsed) else max(0.0, float(default))
    except (TypeError, ValueError):
...
def _normalize_fold_event_metadata(metadata: dict[str, Any]) -> dict[str, Any]:
    """Normalize persisted fold metrics before they affect counters or reports."""
    normalized = dict(metadata or {})
    trigger = normalized.get("trigger")
    normalized["schema_version"] = _nonnegative_int(normalized.get("schema_version"), 1) or 1
    normalized["trigger"] = str(trigger).strip() if isinstance(trigger, str) else "unknown"
    normalized["before_message_count"] = _nonnegative_int(normalized.get("before_message_count"))
    normalized["after_message_count"] = _nonnegative_int(normalized.get("after_message_count"))
    normalized["before_chars"] = _nonnegative_int(normalized.get("before_chars"))
    normalized["after_chars"] = _nonnegative_int(normalized.get("after_chars"))
    normalized["used_fallback"] = _coerce_bool(normalized.get("used_fallback"))
    normalized["duration_ms"] = _nonnegative_float(normalized.get("duration_ms"))
    return normalized
...
def _parse_openai_tool_call(tool_call: dict) -> tuple[str, dict, str | None]:
    """Parse one OpenAI tool call without turning malformed arguments into an empty call."""
    function = tool_call.get("function") if isinstance(tool_call, dict) else None
    if not isinstance(function, dict):
        return "unknown", {}, "Invalid tool call: missing function payload"

    tool_name = str(function.get("name") or "").strip()
    if not tool_name:
        return "unknown", {}, "Invalid tool call: missing function name"

    raw_arguments = function.get("arguments", "{}")
    if raw_arguments is None:
        raw_arguments = "{}"
    if not isinstance(raw_arguments, str):
        return tool_name, {}, f"Invalid tool arguments for {tool_name}: expected a JSON object string"

    try:
        parsed = json.loads(raw_arguments or "{}")
    except json.JSONDecodeError:
        return tool_name, {}, f"Invalid tool arguments for {tool_name}: malformed JSON"

    if not isinstance(parsed, dict):
        return tool_name, {}, f"Invalid tool arguments for {tool_name}: expected a JSON object"
    return tool_name, parsed, None


def _group_openai_tool_batches(checked: list[dict]) -> list[dict]:
    """Group checked OpenAI calls while preserving read/write ordering barriers."""
    batches: list[dict] = []
    for item in checked:
        safe = bool(item.get("allowed")) and item.get("fn") in CONCURRENCY_SAFE_TOOLS
        if safe and batches and batches[-1]["concurrent"]:
            batches[-1]["items"].append(item)
        else:
            batches.append({"concurrent": safe, "items": [item]})
    return batches
...
class Agent:
    def __init__(self,
        ...
        self._fold_fallback_count: int = 0
        self._last_fold_event: dict[str, Any] | None = None
        self._last_fold_used_fallback: bool = False
        self._tool_batch_active: bool = False
        self._pending_compact_reason: str | None = None
    ...
    async def  chat(self, user_message:str)->None:
        ...
        try:
            ...
            # Never let a failed/cancelled tool batch make the next turn look
            # like it is still inside a batch or flush a stale compaction.
            self._tool_batch_active = False
            self._pending_compact_reason = None
   ...
   #执行一次对话，收集本轮模型输出文本，并返回本轮消耗的 token 数
    ...
    def _build_fold_guidance_section(self) -> str:
        ...
        return (
            ...
            f"- Session folds: {self._fold_count} (fallbacks: {self._fold_fallback_count})\n"
    ...
    def _record_fold_event(
        ...
        *,
        ...
        event = _normalize_fold_event_metadata({
            "schema_version": 1,
            "trigger": str(trigger),
        ...
        if event["used_fallback"]:
            self._fold_fallback_count += 1
        self._last_fold_event = event
        self._tool_error_streak = 0
    ...
    def clear_history(self)->None:
        ...
        self._fold_count = 0
        self._fold_fallback_count = 0
        self._last_fold_event = None
        self._last_fold_used_fallback = False
        self._pending_compact_reason = None
        self._tool_error_streak = 0
        self._same_tool_repeat_count = 0
        self._last_tool_name = ""
    ...
    def restore_session(self, data:dict)->None:
        ...
        if data.get("openaiMessages"):
            self._openai_messages = self._normalize_openai_messages(_sanitize_for_utf8(data["openaiMessages"]))
        if isinstance(data.get("foldedSessionMemories"), list):
            restored_records: list[dict[str, Any]] = []
            for raw_record in data["foldedSessionMemories"]:
                if not isinstance(raw_record, dict):
                    continue
                normalized_memory = normalize_folded_memory(raw_record)
                metadata = {
                    key: value
                    for key, value in raw_record.items()
                    if key not in {"episode_memory", "working_memory", "tool_memory"}
                }
                metadata = _normalize_fold_event_metadata(metadata)
                restored_records.append({**metadata, **normalized_memory})
            self._folded_session_memories = _sanitize_for_utf8(restored_records)
            self._fold_count = len(self._folded_session_memories)
            self._fold_fallback_count = sum(
                1 for record in self._folded_session_memories if record.get("used_fallback")
            )
            latest = self._folded_session_memories[-1] if self._folded_session_memories else None
            self._last_fold_event = (
                {
                    key: latest[key]
                    for key in (
                        "schema_version",
...
# 整理 OpenAI 历史消息，补齐被中断的 tool call 批次，避免恢复后的请求违反协议。
    def _normalize_openai_messages(self, messages: list[dict]) -> list[dict]:
        ...
        for raw_message in messages or []:
            ...
            if role == "assistant":
                ...
                if isinstance(raw_calls, list):
                    for raw_call in raw_calls:
                        ...
                        pending_tool_calls[call_id] = raw_call
                if valid_calls:
            ...
            if role == "tool":
                ...
                if not pending_tool_calls or tool_call_id not in pending_tool_calls:
                    continue
                normalized.append(message)
...
#整理 Anthropic 的历史消息，修正部分角色错误，并丢弃不合法的工具调用消息。
        ...
        try:
            save_session(self.session_id, {
                "metadata": {
                    ...
                    "foldCount": self._fold_count,
                    "foldFallbackCount": self._fold_fallback_count,
                    "lastFoldEvent": _sanitize_for_utf8(self._last_fold_event),
                },
    ...
    async def _compact_anthropic(self, *, trigger: str)->bool:
        ...
        if not transcript.strip():
            return False
        before_message_count = len(self._anthropic_messages)
        started = time.perf_counter()
        memory = await self._generate_folded_session_memory(transcript)
        folded_content = format_folded_memory(normalize_folded_memory(memory))
        self._anthropic_messages = [{"role": "user", "content": folded_content}]
        event = self._record_fold_event(
            trigger=trigger,
            before_message_count=before_message_count,
            after_message_count=len(self._anthropic_messages),
            before_chars=len(transcript),
            after_chars=len(folded_content),
            used_fallback=self._last_fold_used_fallback,
            duration_ms=(time.perf_counter() - started) * 1000,
        )
        self._record_folded_session_memory(trigger, memory, event=event)
        self.last_input_token_count = 0
        self._refresh_runtime_system_prompt()
        return True
    ...
    async def _compact_openai(self, *, trigger: str)->bool:
        ...
        if not transcript.strip():
            return False
        before_message_count = len(self._openai_messages)
        started = time.perf_counter()
        memory = await self._generate_folded_session_memory(transcript)
        folded_content = format_folded_memory(normalize_folded_memory(memory))
        self._openai_messages=[
        ...
        event = self._record_fold_event(
            trigger=trigger,
            before_message_count=before_message_count,
            after_message_count=len(self._openai_messages),
            before_chars=len(transcript),
            after_chars=len(folded_content),
            used_fallback=self._last_fold_used_fallback,
            duration_ms=(time.perf_counter() - started) * 1000,
    ...
    async def _generate_folded_session_memory(self, transcript: str) -> dict[str, Any]:
        ...
        if side_query is None:
            self._last_fold_used_fallback = True
    ...
    def _record_folded_session_memory(
        self,
        trigger: str,
        memory: dict[str, Any],
        *,
        ...
        record = {
            ...
            "trigger": trigger,
            "session_id": self.session_id,
            **normalize_folded_memory(memory),
        }
        if event:
            record.update(event)
    ...
    async def _execute_compact_context_tool(self, inp: dict) -> str:
        ...
        if self._tool_batch_active:
            self._pending_compact_reason = reason or "model requested context compaction"
            self._record_tool_outcome("compact_context", True)
            return (
                "Context compaction scheduled after the current tool batch. "
                "Continue returning the remaining tool results first."
            )
    ...
    async def _flush_pending_compaction(self) -> bool:
        """Compact only after all tool results in the current batch are recorded."""
        if not self._pending_compact_reason:
            return False
        self._pending_compact_reason = None
        compacted = await self._compact_conversation(trigger="tool")
        if not compacted:
...
#--------------OpenAI tool batch execution---------------
    async def _execute_openai_tool_batches(self, checked: list[dict]) -> None:
        """Execute one model response's checked calls exactly once.

        Read-only calls may run together, while every mutating or special call
        creates an ordering barrier.  Context compaction is flushed only after
        all results from this response have been appended to the history.
        """
        ...
        try:
            for batch in batches:
                ...
                if batch["concurrent"]:
                    async def _run_oai_safe(ct_item: dict) -> tuple[dict, str]:
                        try:
                            ...
                            raw = f"Error executing tool: {error}"
                ...
                for item in batch["items"]:
                    ...
                    try:
                        raw = await self._execute_tool_call(item["fn"], item["inp"])
                    except Exception as error:
                        raw = f"Error executing tool: {error}"
                    raw = _safe_utf8_text(raw)
                    res = self._persist_large_result(item["fn"], raw)
                    print_tool_result(item["fn"], res)
                    self._record_tool_outcome(
                        item["fn"],
                        not self._looks_like_tool_failure(item["fn"], raw, res),
                    )
        ...
        await self._flush_pending_compaction()

    def _append_openai_skipped_tool_results(self, tool_calls: list[dict], reason: str) -> None:
        """Close an assistant tool-call turn when budget policy stops execution."""
        content = f"Tool execution skipped: {str(reason or 'execution stopped').strip()}"
        for tool_call in tool_calls or []:
            ...
            tool_name = str(function.get("name") or "unknown").strip()
            self._record_tool_outcome(tool_name, False)
            self._openai_messages.append(
                {
                    "role": "tool",
                    "tool_call_id": tool_call_id,
...
#调用 _do()，如果遇到可重试错误，就由 _with_retry() 负责重试。
    ...
    async def _chat_openai(self, user_message:str) -> None:
        ...
        while True:
            ...
            if budget["exceeded"]:
                print_info(f"Budget exceeded: {budget['reason']}")
                self._append_openai_skipped_tool_results(tool_calls, budget["reason"])
                break
            ...
            for tc in tool_calls:
                ...
                fn_name, inp, parse_error = _parse_openai_tool_call(tc)
            ...
            await self._execute_openai_tool_batches(oai_checked)
            self._context_cleared = False
            self._refresh_runtime_system_prompt()
            await self._check_and_compact()
