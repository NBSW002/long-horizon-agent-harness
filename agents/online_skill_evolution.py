...
def validate_online_skill_candidate(candidate: OnlineSkillCandidate | Any) -> list[str]:
    """Validate evidence before a model candidate can enter maintenance."""
    if not isinstance(candidate, OnlineSkillCandidate):
        return ["candidate must be an OnlineSkillCandidate"]
    errors: list[str] = []
    ...
    if not candidate.evidence.strip():
        errors.append("evidence from the user feedback is required")
    return errors


def _coerce_candidate(obj: dict[str, Any]) -> OnlineSkillCandidate | None:
    name = _candidate_text(obj.get("name"))
    description = _candidate_text(obj.get("description"))
    instructions = _candidate_text(obj.get("instructions") or obj.get("prompt"))
    ...
    candidate = OnlineSkillCandidate(
        ...
        when_to_use=_candidate_text(obj.get("when_to_use") or obj.get("when-to-use")),
        instructions=instructions,
        evidence=_candidate_text(obj.get("evidence")),
        tags=tags[:8],
    )
    return candidate if not validate_online_skill_candidate(candidate) else None
...
async def extract_online_skill_candidate(
    *,
    ...
    system = (
        "You are Aegis's online Skill Extractor.\n"
        ...
        "- evidence must quote or clearly summarize the user's durable correction or workflow request; candidates without it are invalid.\n"
...
async def maintain_online_skill_candidate(
    *,
    ...
    system = (
        "You are Aegis's online Skill Set Manager.\n"
