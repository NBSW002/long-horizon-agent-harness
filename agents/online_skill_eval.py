...
REPLAY_SPLITS = frozenset({"mutate_dev", "promotion_test"})
...
def _replay_sample_id(*, skill_name: str, messages: list[dict[str, str]]) -> str:
    return "sample-" + _stable_hash(
        {
...
def _coerce_replay_bool(value: Any, default: bool = True) -> bool:
...
def normalize_replay_sample(
    sample: Mapping[str, Any] | Any,
    *,
    skill_name: str = "",
) -> dict[str, Any] | None:
    """Normalize one online conversation into the versioned replay schema.

    Replay data comes from online logs and is therefore treated as untrusted.
    The evaluator only keeps user/assistant text turns and derives the latest
    fields from those turns so duplicated or stale metadata cannot affect a
    rule result.
    """
    if not isinstance(sample, Mapping):
        return None
    messages = _normalized_messages(sample.get("messages"))
    latest_user = _latest_message(messages, "user")
    if not messages or not latest_user:
        return None

    resolved_skill = str(sample.get("skill") or skill_name).strip()
    if not resolved_skill:
        return None
    split = str(sample.get("split") or "mutate_dev").strip()
    if split not in REPLAY_SPLITS:
        split = "mutate_dev"
    sample_id = str(sample.get("sample_id") or "").strip() or _replay_sample_id(
        skill_name=resolved_skill,
        messages=messages,
    )
    source_type = str(sample.get("source_type") or "unknown").strip() or "unknown"
    return {
        ...
        "ok": _coerce_replay_bool(sample.get("ok", True)),
...
def validate_replay_sample(sample: Mapping[str, Any] | Any) -> list[str]:
    """Return actionable schema errors for a normalized replay sample."""
    if not isinstance(sample, Mapping):
        return ["sample must be an object"]

    errors: list[str] = []
    if sample.get("schema_version") != REPLAY_SCHEMA_VERSION:
        errors.append(f"schema_version must be {REPLAY_SCHEMA_VERSION}")
    for field in ("sample_id", "skill", "source_type"):
        if not isinstance(sample.get(field), str) or not sample.get(field, "").strip():
            errors.append(f"{field} must be a non-empty string")
    if sample.get("split") not in REPLAY_SPLITS:
        errors.append("split must be mutate_dev or promotion_test")
    for field in ("time", "action", "latest_assistant"):
        if not isinstance(sample.get(field), str):
            errors.append(f"{field} must be a string")
    if not isinstance(sample.get("ok"), bool):
        errors.append("ok must be a boolean")
    if not isinstance(sample.get("latest_user"), str) or not sample.get("latest_user", "").strip():
        errors.append("latest_user must be a non-empty string")
    ...
    if not isinstance(messages, list) or not messages:
        ...
        if normalized != messages:
            errors.append("messages must contain only non-empty user/assistant string turns")
        if _latest_message(normalized, "user") != sample.get("latest_user"):
            errors.append("latest_user must match the latest user message")
        if _latest_message(normalized, "assistant") != sample.get("latest_assistant"):
            errors.append("latest_assistant must match the latest assistant message")
    return errors
...
def _build_replay_pool(
    ...
    *,
    ...
    def add_sample(source: dict[str, Any], source_kind: str) -> None:
        normalized = normalize_replay_sample(
            {
                **source,
                "skill": skill_name,
        ...
        if normalized is None:
            return
        samples[str(normalized["sample_id"])] = normalized
...
def _freeze_replay_pool(*, skill_name: str, samples: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for item in existing:
        normalized = normalize_replay_sample(item, skill_name=skill_name)
        if normalized is not None:
            by_id[str(normalized["sample_id"])] = normalized
    ...
    for item in samples:
        ...
        if previous:
            ...
            merged.update({key: value for key, value in normalized.items() if key != "split"})
...
def evaluate_skill_replay(
    *,
    ...
    samples: list[Mapping[str, Any] | Any],
) -> dict[str, Any]:
    """Evaluate a fixed replay set without network calls or filesystem writes.

    This is the deterministic development/validation entry point.  It uses
    only programmatic rules, canonicalizes samples, and deterministically
    assigns ``mutate_dev`` and ``promotion_test`` splits.  LLM judge and
    candidate generation remain in the online async pipeline, where a model
    callback is explicitly supplied.
    """
    snapshot = dict(skill) if isinstance(skill, Mapping) else {}
    skill_name = str(snapshot.get("name") or "offline-skill").strip() or "offline-skill"
    normalized_by_id: dict[str, dict[str, Any]] = {}
    for raw_sample in list(samples or []):
        normalized = normalize_replay_sample(raw_sample, skill_name=skill_name)
        if normalized is not None:
            normalized_by_id[str(normalized["sample_id"])] = normalized
    replay_pool = sorted(
        normalized_by_id.values(),
        key=lambda item: (str(item.get("time") or ""), str(item.get("sample_id") or "")),
        reverse=True,
    )
    replay_pool = _assign_replay_splits(replay_pool)
    rules = _compile_eval_rules(snapshot, include_llm_rules=False)
    evaluation = _summarize_rule_outcomes(rules, replay_pool)
    return {
        "schema_version": REPLAY_SCHEMA_VERSION,
        "mode": "offline_replay",
        "skill": skill_name,
...
def format_skill_replay_markdown(result: Mapping[str, Any] | Any) -> str:
    """Render an offline replay result as a compact, reviewable Markdown report."""
    report = result if isinstance(result, Mapping) else {}
    replay = report.get("replay") if isinstance(report.get("replay"), Mapping) else {}
    evaluation = report.get("eval") if isinstance(report.get("eval"), Mapping) else {}
    lines = [
        "# Offline Skill Replay Evaluation",
        "",
        f"- Skill: `{report.get('skill', '')}`",
        f"- Schema version: `{report.get('schema_version', '')}`",
        f"- Replay samples: `{replay.get('count', 0)}` "
        f"(dev={replay.get('mutate_dev', 0)}, test={replay.get('promotion_test', 0)})",
        f"- Rule pass rate: `{_pct(float(evaluation.get('pass_rate', 0.0) or 0.0))}`",
        f"- Hard failures: `{evaluation.get('hard_failures', 0)}`",
        "",
        "## Rule Summary",
        "",
        "| Rule | Kind | Passed | Total | Pass rate |",
        "|---|---|---:|---:|---:|",
    ]
    by_rule = evaluation.get("by_rule") if isinstance(evaluation.get("by_rule"), list) else []
    if by_rule:
        for item in by_rule:
            if not isinstance(item, Mapping):
                continue
            lines.append(
                f"| {str(item.get('rule_id') or '').replace('|', '/')} "
                f"| {'hard' if item.get('hard') else 'soft'} "
                f"| {item.get('passed', 0)} "
                f"| {item.get('total', 0)} "
                f"| {_pct(float(item.get('pass_rate', 0.0) or 0.0))} |"
            )
    else:
        lines.append("| _No rules evaluated_ | - | 0 | 0 | 0.0% |")
    ...
    if failures:
        ...
        for failure in failures[:20]:
            ...
            reason = str(details.get("reason") or details.get("error") or "unknown failure").strip()
            lines.append(
                f"- `{failure.get('sample_id', '')}` / `{failure.get('rule_id', '')}`: {reason}"
            )
    return "\n".join(lines).rstrip() + "\n"


def write_skill_replay_reports(
    result: Mapping[str, Any] | Any,
    *,
    json_path: str | Path,
    markdown_path: str | Path,
) -> tuple[Path, Path]:
    """Explicitly write the JSON and Markdown forms of an offline result."""
    ...
    markdown_target.write_text(
        format_skill_replay_markdown(result),
        encoding="utf-8",
    )
...
async def _evaluate_online_skill_evolution_core(
    *,
    ...
    report = {
        ...
        "execution_mode": "llm_augmented" if side_query is not None else "offline_programmatic",
        "data_dir": str(root),
        "methodology": {
...
def format_online_skill_eval(report: dict[str, Any] | None = None) -> str:
    ...
    lines = [
        ...
        f"  execution_mode={report.get('execution_mode', 'offline_programmatic')}",
        (
            "  aggregate: "
