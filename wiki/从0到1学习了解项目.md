# Aegis 从 0 到 1 学习了解项目

这是一份面向学习 Aegis 项目的完整说明文档。它的目标不是只列模块名，而是帮助你从零开始理解：这个项目解决什么问题、为什么要这样设计、一次用户请求在系统里如何流转、每个核心模块承担什么职责，以及你应该怎样继续扩展它。
...
- 想把 Aegis 作为简历项目、答辩项目或课程项目讲清楚的人。
...
Aegis 是一个用 Python 实现的命令行 Coding Agent。你可以把它理解成一个“本地运行的 Agent Runtime”：用户在终端输入任务，Agent 调用大模型理解需求，大模型可以发起工具调用，Runtime 负责检查权限、执行工具、把结果回写给模型，并持续循环直到给出最终回复。
...
> Aegis 是一个用于学习和改造的本地 Coding Agent Runtime，核心价值是把模型推理、工具执行、权限控制、长期记忆、Skills、自进化和外部工具扩展串成一个可运行闭环。

### 1.2 先看总架构

![Aegis 总体架构](assets/architecture/01-overall-architecture.svg)
...
![Aegis 学习路线](assets/architecture/08-learning-roadmap.svg)
...
| 终端 | 用途 | 常用命令 |
|------|------|----------|
| 运行终端 | 启动 Aegis | `python3 -m agents.main` |
| 源码终端 | 搜索主链路 | `rg -n "async def chat|_execute_tool_call|check_permission" agents` |
| 审计终端 | 观察持久化结果 | `tail -f .bear/skill-evolution/online_provenance.jsonl` |
...
![Aegis Agent Loop 主链路](assets/architecture/02-chat-flow.svg)
...
Aegis 的 `agents/agent.py` 就是实现这些逻辑的地方。
...
这里最值得学习的是 pending window 设计。Aegis 不是只看当前用户输入就立刻沉淀 Skill，而是把“上一轮用户任务 + 上一轮助手输出 + 下一轮用户反馈”组合起来判断。这样可以更准确地区分一次性内容和稳定偏好。
...
| Prompt 控制就是安全控制 | Prompt 是软约束，`check_permission()` 才是硬约束 |
...
system prompt 是模型每轮推理的“运行说明书”。Aegis 的 prompt 不只是几句身份设定，它会动态注入当前环境、项目规则、Memory、Skills、子 Agent 类型和延迟工具提示。
...
因此 Aegis 在工具执行前统一走 `check_permission()`。
...
Aegis 要求模型编辑已有文件前必须先 `read_file`。实现方式是 `read_file_state`：
...
在 Aegis 里，Skill 是一个可复用能力说明文件，通常是一个 `SKILL.md`。它保存的不是某一次任务的输出，而是可以反复应用的方法、规则、偏好或工作流。
...
为什么要检索？因为不能把所有 Skill 全量塞给模型。Skill 多了以后，全量注入会浪费上下文，也会干扰模型判断。所以 Aegis 每轮只检索与当前问题最相关的几个 Skill。
...
Aegis 的自进化目标是：从用户明确反馈中抽取未来可复用的规则，并将其新增为 Skill 或合并进已有 Skill。
...
![Aegis Skills 自进化架构](assets/architecture/03-agent-skills.svg)

更细的演化流程可以看：

![Aegis Skills 自进化流程](assets/architecture/07-skills-evolution-flow.svg)
...
![Aegis 自进化审计闭环](assets/architecture/05-monitor-eval.svg)
...
- Aegis：在这里扮演 MCP Client，把 MCP Server 暴露的工具包装成 Agent 可调用工具。

为什么需要 MCP？

如果没有 MCP，每接一个外部系统都要写一套专用集成。例如 GitHub 一套、数据库一套、浏览器一套、文件系统一套。MCP 的价值是提供统一协议：外部能力只要实现 MCP Server，模型应用就能用统一方式发现和调用。

![MCP 概念与 Aegis 接入流程](assets/architecture/11-mcp-concept-flow.svg)
...
- Aegis 向它的 stdin 写 JSON-RPC 请求。
- Aegis 从它的 stdout 按行读取 JSON-RPC 响应。
- 每个请求有递增 id。
...
![Aegis 数据与存储架构](assets/architecture/04-data-storage.svg)
...
如果要把 Aegis 作为课程或简历项目展示，推荐提交 `wiki/`、示例 `.bear/skills/` 和 `.bear/rules/`，但不要提交真实 `.env`、个人 Memory、Session 和敏感审计数据。
...
![Aegis 部署结构](assets/architecture/06-deployment.svg)
...
4. 阅读 `tools.check_permission()`，解释五种权限模式。
5. 阅读 `prompt.build_system_prompt()`，说清 system prompt 动态注入了哪些上下文。
...
4. 查看 `.bear/skill-evolution/usage.jsonl`，理解 create、invoke、feedback、evolve 事件。

长期能力：

1. 手动创建一个项目级 Skill，并用 `/skills` 查看。
2. 用 `/skill-evolve` 演化 Skill，查看 `history` 和 `usage.jsonl`。
3. 手动运行 `/extract_now`，查看 `online_provenance.jsonl`。
...
如果需要向别人介绍 Aegis，可以按下面顺序讲。

第一段讲定位：

```text
Aegis 是一个用 Python 实现的本地 Coding Agent Runtime。它不是单纯聊天 CLI，而是把模型推理、工具调用、权限控制、长期记忆、Skills、自进化、MCP、子 Agent 和会话恢复串成一个可运行闭环。
```

第二段讲主链路：

```text
用户输入进入 main.py，main.py 解析参数和环境变量后创建 Agent。Agent.chat() 构建动态 prompt，检索 Skills，预取 Memory，然后调用 Anthropic-compatible 或 OpenAI-compatible 模型。模型如果返回 tool call，Runtime 会先做权限判断，再执行工具，把结果回写给模型，直到模型给出最终回答。
```

第三段讲安全边界：

```text
项目不是只靠 prompt 约束模型，而是在工具执行层实现了权限模式、Plan Mode、危险命令检查、settings allow / deny、编辑前读取和 mtime 校验。模型只能提出意图，真正执行动作的是 Runtime。
```

第四段讲长期能力：

```text
Memory 保存长期事实和用户偏好，Skills 保存可复用方法。Skills 还支持在线自进化：系统会用上一轮任务、助手输出和下一轮用户反馈组成 pending window，再通过 Extractor 和 Maintainer 决定新增、合并或丢弃 Skill，并保留 provenance、history 和 usage stats。
```
