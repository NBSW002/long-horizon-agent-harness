...
本文档说明 Aegis 的在线 Skills 评测是怎么做的。重点解释每个阶段的名词含义、代码流程、输入输出和当前边界。
...
为了让开发集和验证集能够在没有真实模型、没有网络的情况下重复运行，新增了纯程序化入口：

```python
from agents.online_skill_eval import evaluate_skill_replay
    ...
    samples=[
        {
            "messages": [
                ...
                {"role": "assistant", "content": "Sources: https://example.com\nIt changed."},
            ]
        }
    ],
)
...
该入口不会调用 LLM，也不会写入 `.bear/skill-evolution/`。它会先把输入规范化为 `schema_version=1` 的固定样本格式，再根据样本 ID 稳定划分 `mutate_dev` 和 `promotion_test`，最后执行程序化规则。相同的 Skill 和 Replay 输入会得到相同的规则、分组和判断结果，适合先写回归测试，再进行候选 Skill 改造。
...
`normalize_replay_sample()` 会丢弃缺少用户消息、非字符串消息和非法角色；`validate_replay_sample()` 用于在写入数据集前报告字段错误。在线评测也复用这套规范化逻辑，因此离线开发集和线上固化的 Replay 数据格式一致。

项目还提供了无需编写 Python 调用代码的标准命令：
...
命令会生成 `result.json` 和 `result.md`。它只评估当前输入样本，不会创建或覆盖 active `SKILL.md`，也不会写入 champion。
...
```text
Online skill eval:
  data_dir=<project-root>/.bear/skill-evolution
  aggregate: ingests=8, ok_rate=100.0%, candidate_events=1, acceptance_rate=100.0%, replay_samples=1, rule_pass_rate=50.0%, llm=on, llm_rules=4, llm_judgments=1, llm_pass_rate=0.0%, candidates=1
  actions: none=7, add=1, merge=0, discard=0, failed=0, denied=0
  statuses: incubating=3, unobserved=1
  champion_statuses: incubating=3, unobserved=1
  ...
  report_file=<project-root>/.bear/skill-evolution/online_eval_report.json
```
...
```text
<project-root>/.bear/skill-evolution
```
...
```text
<project-root>/.bear/skill-evolution/online_eval_report.json
```
